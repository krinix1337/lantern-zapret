...
> 
> *Дисклеймер*: Проект **Lantern (lantern-zapret)** является независимым Open Source приложением для обхода DPI в РФ и никак не связан с проектом Lantern VPN (getlantern.org).
...
> [!TIP]
> **Если Windows SmartScreen выдает предупреждение:**  
> Так как программа бесплатная и с открытым исходным кодом, у нее нет платного коммерческого сертификата Microsoft за $500+/год. Для запуска нажмите **«Подробнее» → «Выполнить в любом случае»**. Подлинность сборки всегда можно проверить по хэшу `Lantern-Setup.exe.sha256` из релиза.
...
└── .github/workflows/  # 🤖 Сборка релиза: установщик и файл контрольной суммы SHA256
```
...
> при установке они попадают в `<папка программы>\zapret\`, при работе
> из исходников — в корень репозитория. Готовый установщик
> `Lantern-Setup.exe` и контрольная сумма `.sha256` тоже не хранятся в репозитории:
> их автоматически собирает workflow `Release` и публикует как ассеты релиза.
