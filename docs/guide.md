...
3. [🎵 Встроенный музыкальный плеер](#-встроенный-музыкальный-плеер)
4. [🧩 Каталог стратегий и быстрое редактирование](#-каталог-стратегий-и-быстрое-редактирование)
5. [🔎 Проверка соединения и бенчмарк](#-проверка-соединения-и-бенчмарк)
6. [⚙️ Служба Windows (Автозапуск)](#️-служба-windows-автозапуск)
7. [🎛️ Управление фильтрами и списками](#️-управление-фильтрами-и-списками)
8. [⚙️ Настройки, Watchdog и профили](#️-настройки-watchdog-и-профили)
9. [📋 Журнал событий и диагностика](#-журнал-событий-и-диагностика)
10. [ℹ️ Безопасность и проверка целостности](#ℹ️-безопасность-и-проверка-целостности)
11. [❓ Решение частых проблем (FAQ)](#-решение-частых-проблем-faq)
...
<img src="screens/overview.png" width="900" alt="Главный экран — Обзор">
...
В левой боковой панели Lantern расположен стильный встроенный медиаплеер для прослушивания музыки во время работы или игр.

### Возможности плеера:
1. **Мультиформатная поддержка**:
   - Воспроизведение треков `.mp3`, `.wav`, `.m4a`, `.aac`, `.flac`, `.wma`, `.ogg`, `.mp4`.
2. **Интеллектуальное чтение тегов и обложек (`AudioTagReader`)**:
   - Автоматически извлекает обложки альбомов из ID3v2 (APIC/PIC), MP4 атомов (`covr`), FLAC picture блоков или файлов `cover.jpg` / `folder.png`.
   - Если обложка отсутствует, отображается фирменный арт.
3. **Плавный аудио- и видео-кроссфейд**:
   - Мягкое синусоидальное угасание звучания текущей композиции и нарастание новой строго при готовности декодера `MediaOpened`.
   - Плавное визуальное растворение и появление обложки и названия трека.
4. **Управление громкостью и воспроизведением**:
   - Плавный регулятор громкости с защитой от выхода за границы, быстрый Mute / Unmute, регулировка колёсиком мыши по карточке плеера.
   - Режим случайного воспроизведения **Shuffle 🔀**, перемотка по SeekBar, переключение треков ⏮️/⏭️ и кнопка скрытия.
   - Включение и отключение отображения плеера одной кнопкой в **Настройки → Интерфейс**.
...
<img src="screens/strategies.png" width="900" alt="Раздел Стратегии">
...
3. **⚙️ Быстрое редактирование в Блокноте**:
...
4. **🌐 Умная рекомендация по провайдеру (ISP)**:
...
<img src="screens/check.png" width="900" alt="Раздел Проверка соединения">
...
<img src="screens/service.png" width="900" alt="Раздел Служба Windows">
...
2. **Единое управление автозапуском**:
...
<img src="screens/filters.png" width="900" alt="Раздел Фильтры">
...
## ⚙️ Настройки, оптимизация и профили
...
<img src="screens/settings.png" width="900" alt="Раздел Настройки">

</div>

### Ключевые возможности:
- **🐶 Watchdog (Автопереключение при сбое)**:
  В фоновом режиме каждые $N$ минут опрашивает ключевые эндпоинты Discord и YouTube. Если провайдер заблокировал текущую стратегию, Watchdog автоматически переберет каталог стратегий и включит рабочую с сохранением режима работы службы.
- **⚡ Экстремальная оптимизация ОЗУ (RAM)**:
  ...
  - Ленивая загрузка медиаплеера: движок воспроизведения инициализируется только при первом старте музыки, экономя до 70 МБ ОЗУ при обычном использовании.
- **🎬 Кинематографичные переходы и сохранение позиции**:
  - Плавный бесшовный переход между темами и языками с эффектом размытия (`BlurEffect` от 6px до 0) и затухания (`Fade`).
  - Позиция прокрутки страниц сохраняется с точностью до пикселя при смене тем и языков.
  - Полная поддержка режима «Уменьшение движения» (`reduce_motion`): анимации мгновенно отключаются при активации соответствующей системной опции.
...
<img src="screens/log.png" width="900" alt="Раздел Лог событий">
...
<img src="screens/about.png" width="900" alt="Раздел О программе">

</div>

- **Проверка контрольных сумм SHA-256**: в один клик сверяет хэши исполняемых файлов с официальным манифестом `SHA256SUMS.txt`.
...
<details>
<summary><b>4. Предупреждение Windows SmartScreen («Защитник Windows защитил ваш компьютер»)</b></summary>
...
<details>
<summary><b>5. Антивирус блокирует драйвер WinDivert64.sys или папку приложения</b></summary>
